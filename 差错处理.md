\5) 课程时间解析 `ProcessRow()`/`isValidSchedule()` 对“节次”为**单数字**的假设过强**建议**：用更稳健的解析（正则或手动扫描数字段）：

- 正则示例：`^(\d{1,2})-(\d{1,2})节$`；
- 解析后再做范围校验（1–12 或项目定义的上限），再遍历置位。

6) `isValidSectionId()` 条件含义略显晦涩 & 同样受拼接问题影响

**问题**：同样存在字符串拼接无空格；另外条件
 `MONTH(sm.start_date) >= IF(MONTH(NOW()) >= 7, 7, 0)`
 可读性较差且业务意义不直观（疑似想区分春/秋学期）。

**问题**`DataValidator` 若用于高并发，`timetable` 全局数组需并发隔离

**现状**：`bool timetable[22][8][9];` 是**进程级**可变全局；并发检测多用户/多教师时会互相污染。

### 总结

**引入数据库抽象层**：如 `DbHandler`，使得系统对数据库的操作通过统一接口进行，简化数据库切换的工作。这样只需要针对具体数据库实现相应的类，而不需要修改业务逻辑部分。

**缓存机制**：对于一些重复查询的数据（如课程信息、成绩等），引入缓存机制可以显著提高性能。

**权限管理**：可以进一步考虑对每个请求方法加入权限验证，尤其是涉及数据修改的操作，以增强安全性。
...
数据库：本学期的课，< deadline 被注释掉了方便测试
人数 < max_capacity，6.学院条件
先修课，该课已通过
